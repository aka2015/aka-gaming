    ...
    async function publishGame(id: string) {
        ...
        if (res.ok) {
            setGames((prev) =>
                prev.map((g) =>
                    g.id === id
                        ? { ...g, status: "published", publishedAt: Date.now() }
                        : g,
                ),
            );
        }
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">
                        Maksimal 3 game ({games.length}/3)
                    </p>
                </div>
                {games.length < 3 && (
                    <Link
                        href="/create"
                        className="btn-primary-custom no-underline text-sm"
                    >
                        + Buat Game Baru
                    </Link>
                )}
            ...
            {loading ? (
                ...
                <div className="space-y-4">
                    {games.map((game) => (
                        <div
                            key={game.id}
                            className="bg-white rounded-xl shadow-sm p-5 flex items-center gap-4 border border-gray-100"
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex gap-2 mt-1 flex-wrap items-center">
                                    <span
                                        className={`text-xs font-bold px-2 py-0.5 rounded-full ${game.status === "published" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"}`}
                                    >
                            ...
                            <div className="flex gap-2">
                                {game.status === "draft" && (
                                    <button
                                        ...
                                        className="px-3 py-1.5 bg-green-500 text-white rounded-full text-xs font-bold hover:bg-green-600 transition"
                                    >
                                        🚀 Publish
                                    </button>
                                )}
