...
export async function getUserCredits(userId: string): Promise<{
    ...
    const checkinStreak = data.checkinStreak ?? 0;
    ...
    return adminDb.runTransaction(async (transaction) => {
        ...
        if (lastReset !== today) {
            const rollover = Math.min(
                credits,
                CREDIT_CONFIG.MAX_CREDIT_ROLLOVER,
            );
            credits = rollover + CREDIT_CONFIG.DAILY_CREDITS;
        ...
        if (credits < amount) {
            return {
                success: false,
                credits,
                error: "Credit tidak cukup. Tonton ads atau tunggu reset besok.",
            };
        }
...
export async function addCreditsFromAd(userId: string): Promise<{
    success: boolean;
    credits: number;
    adsWatchedToday: number;
    error?: string;
    ...
    return adminDb.runTransaction(async (transaction) => {
        ...
        if (adsWatchedToday >= CREDIT_CONFIG.MAX_ADS_PER_DAY) {
            return {
                ...
                credits,
                adsWatchedToday,
                error: "Batas nonton ads hari ini sudah tercapai",
            };
        }
...
export async function claimDailyCheckin(userId: string): Promise<{
    ...
    return adminDb.runTransaction(async (transaction) => {
        ...
        const rewardIndex = Math.min(
            streak - 1,
            CREDIT_CONFIG.CHECKIN_REWARDS.length - 1,
        );
