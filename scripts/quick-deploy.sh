#!/bin/bash
# Quick deploy script - run this on the VPS
...
echo " Location: /var/www/games/tower-defense-pro/"
echo ""
echo "Now add to Firestore:"
echo "cd /opt/aka-gaming && node -e \"
const admin = require('firebase-admin');
admin.initializeApp({ credential: admin.credential.cert(require('./service-account.json')) });
admin.firestore().collection('games').doc('tower-defense-pro').set({
  title: 'Tower Defense Pro',
  description: 'Defend your base with 4 unique towers!',
